...
def read2list(filename, **kwargs):
    ...
    @keyword splitchar: character seperating entries in a row
                        (default: whitespace)
...
def read2recarray(filename, **kwargs):
    ...
    # -- if dtypes is None, we have some room to automatically detect the
    #   contents of the columns. This is not fully implemented yet, and works
    #   only if the second-to-last and last columns of the comments denote the
...
def write_array(data, filename, **kwargs):
    ...
    # use '%g' or '%f' or '%e' for writing floats automatically from record
    # arrays with auto width
    use_float = kwargs.get('use_float', '%f')

    # -- switch to rows first if a list of columns is given
    if not isinstance(data, np.ndarray):
        data = np.array(data)
    if 'row' not in axis0.lower():
        data = data.T

    if formats is None:
        try:
            formats = [('S' in str(data[col].dtype) and '%s' or use_float)
                       for col in data.dtype.names]
        except TypeError:
            formats = [('S' in str(col.dtype) and '%s' or '%s')
                       for col in data.T]
    ...
    if auto_width is True and header == True:
        for fmt, head in zip(formats, data.dtype.names):
            col_widths.append(max([len('%s' % (fmt) % (el))
                                   for el in data[head]] + [len(head)]))
    # -- for normal arrays and specified header
    elif auto_width is True and header is not None:
        for i, head in enumerate(header):
            col_widths.append(max([len('%s' % (formats[i]) % (el))
                                   for el in data[:, i]] + [len(head)]))
    # -- for normal arrays without header
    elif auto_width is True and header is not None:
        for i in range(data.shape[1]):
            col_widths.append(
                max([len('%s' % (formats[i]) % (el)) for el in data[:, i]]))
    ...
    if header is not None and col_widths:
        ff.write('#' + sep.join(['%%%s%ss' % (('s' in fmt and '-' or ''), cw)
                                 % (head) for fmt, head, cw in
                                 zip(formats, header, col_widths)]) + '\n')
    ...
    if col_fmts is not None and col_widths:
        ff.write('#' + sep.join(['%%%s%ss' % (('s' in fmt and '-' or ''), cw)
                                % (colfmt) for fmt, colfmt, cw in
                                zip(formats, col_fmts, col_widths)]) + '\n')
    elif col_fmts is not None:
        ff.write('#' + sep.join(['%%%ss' % ('s' in fmt and '-' or '') %
                                (colfmt) for fmt, colfmt in
                                zip(formats, col_fmts)]) + '\n')

    # -- WRITE DATA
    # -- with automatic width
    if col_widths:
        for row in data:
            ff.write(' ' + sep.join(['%%%s%s%s' % (('s' in fmt and '-' or ''),
                                cw, fmt[1:]) % (col) for col, cw, fmt in
                                zip(row, col_widths, formats)]) + '\n')
...
def read_photrot(infile, teff=None, logg=None):
    ...
    if there is a NAD file with same name (but extension .nad instead of .phot)
   ...
   @param teff: range in eff. temperature for file selection (value, sigma)
   @type teff: tuple
   ...
   """
    ...
    with open(infile, 'r') as f:
        ...
        while 1 and valid_funds:
            ...
            if len(line.strip()) == 0:
                ...
                # reading the 50 next lines which give vector B and associated
                # l-values
                l_B_, B_ = line.split()
                d['B'].append(float(B_))
    ...
    logger.debug("Read %d modes from MADROT %s (%d modes failed)" %
                 (len(blocks), infile, nrfails))
...
def read_nad(nadfile, only_header=False):
    ...
    with open(nadfile, 'r') as ff:
        ...
        while 1:
            ...
            line = line.strip()
            # general header containing star info
            if line[0] == '#' and lines <= 3:
                ...
                for i in range(len(contents)):
                    ...
                    if entry_value is not None:
                        ...
                        star_info[entry_name.lower().replace(' ', '')
                                  ] = entry_value
                        entry_name = None
                        entry_value = None
                    else:
                        entry_name += contents[i]
            # we were only interested in the header
            elif only_header and lines > 3:
                break
            elif line[0] == '#':
                myheader = [head.strip()
                            for head in line[1:].split('  ') if head]
            else:
                line = line.split()
                thismode = {}
                ...
                if not 'm' in myheader:
                    thismode['m'] = 0
                # -- for rotating AND nonrotating nad files, omega_rot is not
                # available

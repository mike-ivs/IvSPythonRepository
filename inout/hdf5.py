...
def write_dict(data, filename, update=True, attr_types=[]):
    ...
    contain other nested dictionaries, this file stucture will be maintained in
    the saved hdf5 file.

    Pay attention to the fact that the data type of lists might change when
    writing to hdf5. Lists are stored as numpy arrays, thus all items in a list
    are converted to the same type: ['bla', 1, 24.5] will become ['bla', '1',
    '24.5']. Upt till now there is nothing in place to check this, or correct
    it when reading a hdf5 file.

    @param data: the dictionary to write to file
    @type data: dict
    @param filename: the name of the hdf5 file to write to
    @type filename: str
    @param update: True if you want to update existing file, False to overwrite
    @type update: bool
    @param attr_types: data types that you want to save as attribute instead
                       of a dataset. (standard everything is saved as dataset.)
    ...
    def save_rec(data, hdf):
        ...
        for key in list(data.keys()):
            ...
            if type(data[key]) == dict:
                # if part is dictionary: add 1 level and save dictionary in new
                # level
                if key not in hdf:
